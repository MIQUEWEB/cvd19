...
class NewItemTest(TestCase):
    ...
    def test_redirects_to_list_view(self):        
        other_list = COVIDList.objects.create()        
        correct_list = COVIDList.objects.create()        
        response = self.client.post(f'/RICList/{correct_list.id}/add_item',data={'pet': 'A new pet','owner': 'A new owner','address':'A new address','breed': 'A new breed','birthday': 'A new birtday'})  
...
"""class HomePageTest(TestCase):
  ...
  def test_save_POST_request(self):
    ...
    self.assertIn('flname', response.content.decode())
